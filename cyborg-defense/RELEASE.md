...
## 再開するときの覚書（4.3(a) をどう避けるか）

**同じ内容で出し直しても同じ結果になる可能性が高い。** 実測して分かっている事実:

- 中身で反論しても通らなかった（KeyTag は 1,153字の審査ノートで具体反論 → 同じ定型文で再リジェクト）
- **既に配信中のアプリの小さな更新まで 4.3(a)** になった（スクラップメモ 1.0.5）
  ＝ 個々のアプリの独自性の説明では説明が付かない

したがって検討すべきは「文言の直し方」ではなく**出し方そのもの**:

| 案 | 中身 |
|---|---|
| 本数を絞る | 4本（にゃんこ大脱出／カラー重力／サイボーグ防衛軍／ひよこ防衛軍）を**同時に出さない**。1本に絞り、結果を見てから次 |
| 1本にまとめる | 小さなゲームを**1つのアプリに同梱**する（ゲーム集）。殻が同じでも「別アプリの量産」に見えない |
| 間隔を空ける | 6時間に3本のような出し方をしない |
| 殻の差を出す | Capacitor の既定のまま出すと**実行ファイルがほぼ同一**になる。ネイティブ側に固有の実装を持たせるか、Web版だけで公開する |
| Web版で出す | GitHub Pages では既に公開済み（審査不要）。**App Store に出さない**という選択も現実的 |

**判断はオーナー。** 決まるまで、このフォルダからは何も外へ出さない。
