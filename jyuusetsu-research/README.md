...
- 直し方は2案。**推奨は②**（課金の当たらない Embed 専用キーを別に作る）
  1. 既存の Web キーのリファラ許可に `http://localhost:*` / `http://127.0.0.1:*` /
     `http://192.168.1.105:*` を足す … 手軽だが、そのキーは Maps Static（有料）も通る
  2. **社内画面用に Maps Embed だけに絞ったキーを新規作成**する … 埋め込みは無制限無料なので、
     万一漏れても課金が発生しない
