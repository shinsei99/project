...
def _check_input(data: LegalCrossCheckData) -> list[CheckResult]:
    ...
    if owner or seller_exp or seller_con:
        ...
        results.append(CheckResult(
            ...
            # 謄本に所有者名があるだけでは「一致」にしない。
            # 元の実装は `STATUS_OK if owner` だったため、重説・契約書の売主欄が
            # 両方とも空でも🟢一致と出ていた（何とも照合していないのに一致と言う）。
            # 検閲ツールで偽の緑は見落としを生むので、相手側の値が要る。
            status=(STATUS_NG if ng
                    else (STATUS_OK if (owner and (seller_exp or seller_con)) else STATUS_NA)),
