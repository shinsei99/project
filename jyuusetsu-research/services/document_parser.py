...
def _num_near(text: str, label_pat: str) -> float | None:
    ...
    ラベルは必ず非キャプチャ群で囲む。`建ぺい率|建蔽率` のような選択を素で連結すると
    正規表現が `建ぺい率` または `建蔽率[^\\d]{0,12}(...)` と解釈され、
    前者に当たったとき group(1) が None になって落ちる
    （建ぺい率の記載がある重説なら必ずクラッシュしていた。2026-08-21 修正）。
    """
    m = re.search(r"(?:" + label_pat + r")[^\d]{0,12}([\d,，]+(?:\.\d+)?)", text)
