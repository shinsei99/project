...
def _ensure_template() -> str:
    ...
    `templates/*.xlsx` は gitignore なので、別PCで clone した直後は存在しない。
    生成スクリプトは committed なので、その場で作れば済む（人の手作業を増やさない）。
    """
    if not os.path.exists(_TEMPLATE_PATH):
        import runpy
        gen = os.path.join(os.path.dirname(_TEMPLATE_PATH), "generate_crosscheck_template.py")
