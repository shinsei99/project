...
#
# 使い方:
#   ./screenshots/shoot.sh start play stack clear gameover          # iPhone
#   DEVICE=ipad ./screenshots/shoot.sh start play stack clear gameover
set -euo pipefail
cd /Users/apple/nyanko-ice
...
for SHOT in "$@"; do
  echo "── $SHOT ($DEVICE) ──"
  # www/ の正本に、撮影用の細工を IIFE の中（末尾の })(); の直前）へ差し込む
  python3 - "$SHOT" <<'PY'
import sys
shot = sys.argv[1]
...
PY
  ...
  # ★このアプリは Capacitor 6＝CocoaPods なので **workspace** で建てる。
  #   -project だと AppDelegate.swift が `unable to resolve module dependency: 'Capacitor'` で落ちる
  #   （KeyTag は Capacitor 8＝SPM なので -project でよい。アプリごとに違う）
