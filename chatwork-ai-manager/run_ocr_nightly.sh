...
#
#   ★なぜ速くなったか＝**OCRを二段構えにしたから**（2026-08-31 `b674b7ea`
#   「macOS Vision → 取れなければ claude vision」）。Vision は無料・通信不要・2.3秒/ページ。
#   下の「実測186件/晩」は**claude単独だった頃の数字**なので、もう当てにしない
#   （9/4の実測は1件あたり約1.5秒＝ほぼ全件が Vision で完了）。
#
#   ただし**「上げても無料」ではない**。Vision で読めなかったぶんだけ claude へ回るので、
#   手書き帳票の束に当たる晩は枠を食う（9/2 は走査した新規24件が全部 claude 行きだった）。
#   残り約3,300件 → 3〜4晩で終わる見込み。**終わったら300へ戻してよい。**
