...
日報（社員ごと・毎日・実務の記録）とは役割も起点も別物:
  ...
  - 内容: 個々の業務の進捗ではなく、**会社としての方針・戦略**を残す位置づけ。
...
会社としての月報（方針・戦略の記録）を書いてください。

# 絶対に守ること
- **会議資料・会議内容に書かれていないことは書かない。** 想像で方針を作らない。
- 推測せざるを得ないことは文末に「（推測）」と付ける。
- 日報（社員ごとの日々の実務記録）とは役割が違います。**個々の業務の進捗ではなく、
  会社としての大きな方向性・方針・決定事項**を中心にまとめてください。
- 添付資料のうち抽出に失敗しているものがあれば、内容を想像で埋めず、
  「そのほか」等に「〇〇（ファイル名）は内容を読み取れませんでした」とだけ書く。
...
body_md は**次の4つの見出しだけ**を、この順で書いてください。見出しを増やさない。
