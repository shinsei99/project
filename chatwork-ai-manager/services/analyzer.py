...
8. 分割した案件ごとの報告が、既存の未完了TODO（一覧参照）のどれにも該当しない場合
   ...
   **ただし、外観写真だけの定例巡回報告（ルール9）にはこの規則を適用しない**
   （巡回報告は「事前登録の無かった業務の報告」であっても new_task にしない）。
9. **写真（外観写真等）だけ、または写真＋一言（「〇〇を巡回しました」「〇〇の外観です」等）
   の、定例の巡回・現地確認の報告**は、それ自体を new_task にしない。巡回の実施は
   「誰かが新しく何かをする必要がある」新規依頼ではなく、すでに割り当て済みの巡回業務
   （既存TODO・上司からの定例指示等）の実施記録にすぎないため。
   - 報告者の**包括的な巡回TODO**（例:「〇〇さんの担当物件巡回・確認作業」のように、
     個別の物件名ではなく担当者単位で登録されているTODO）が既存の未完了TODO一覧にあれば、
     報告された物件名がTODOの文言に完全一致しなくても target_task_id でそこへ紐付け、
     event_type="progress_update" とする（物件ごとに new_task へ分割しない。ルール7より優先）。
   - 該当する包括的な巡回TODOが未完了TODO一覧に見当たらない場合（最初から登録が無い、
     既に完了扱いになっている等）は、event_type="none" とし、new_task も作らない。
   - ただし、巡回中に破損・違法駐輪・要修繕箇所など**対応が必要な具体的な問題が見つかった**
     場合は、その問題への対応自体を new_task にしてよい（巡回の実施報告ではなく、
     見つかった問題への対応が新しい業務になるため）。
