...
- {job_type}=carryover_1000 は「前日以前が期限のまま未完了（期限超過）」と「期限未設定かつ一度もAI確認していない（AI確認待ち）」の2種類が混在する。期限超過は催促、期限未設定・未確認は「進捗はいかがですか」という穏やかな確認にする（催促口調にしない）。progress_1300 は昼の進捗確認。役割に合った文面にする。"""
