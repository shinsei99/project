...
# ★巡回写真を物件フォルダへ入れる（2026-09-05 オーナー指示で追加）。
#   ① Chatwork から画像の実体を取り直す（data/web_images は一時置き場なので消える）
#   ② 物件フォルダの 記録・写真/巡回/ へ振り分ける
#   2026-09-05 に数えたら 84枚のうちフォルダに入っていたのは9枚だけだった。
#   ★新誠プロパティの物件は save 側で弾いている（大京の共有フォルダには置かない）。
#   ★失敗しても棚卸しは巻き添えにしない。
/usr/bin/python3 tools/fetch_patrol_photos.py --go >>"$LOG" 2>&1
frc=$?
/usr/bin/python3 tools/save_patrol_photos.py --go >>"$LOG" 2>&1
src=$?
if [ "$frc" -ne 0 ] || [ "$src" -ne 0 ]; then
  log "★巡回写真の取り込みに失敗（取得 ${frc} / 保存 ${src}）。棚卸しは成功しているので続行する"
else
  log "巡回写真を物件フォルダへ入れた"
fi

# ★年度計画表のチェックを更新する（2026-09-05 追加）。
#   業者の報告書フォルダを読み直して「✓ 実施済み／未／色分け」を今日の日付で付け直す。
#   ここに相乗りさせる理由は、棚卸しと同じく **Dropbox を読む仕事だから**
#   （/bin/bash 経由でしか読めない。専用の launchd を1本増やす必要が無い）。
#   年度は check_yearplan.py が今日から自動で決める（4〜3月）。手で直す箇所は無い。
#   ★失敗しても棚卸しは巻き添えにしない（計画表が古いままでも実害が小さいため）。
/usr/bin/python3 tools/check_yearplan.py --go >>"$LOG" 2>&1
prc=$?
if [ "$prc" -ne 0 ]; then
  log "★年度計画表のチェック更新に失敗（終了コード ${prc}）。棚卸しは成功しているので続行する"
else
  log "年度計画表のチェックを更新した"
fi
