...
- 18:00にはさらに**業務記録リマインド**を同居させている（2026-08-27）。その日Chatworkに投稿した発言数（業務日報が本文生成に使う数え方と同じ）が既定3件未満の社員へ、18:30の業務日報自動生成より前に入力・報告を促す。設定は管理画面「システム設定」→「業務記録リマインド」。
...
## 物件×担当者マスタ → TODO自動担当割当 ※2026-08-26 追加（TASK-20260826-003）
台帳の「担当」列（`properties.assignee_name`）を正式な物件×担当者マスタとして扱い、
`services/analyzer.py`のTODO抽出・`agent_tools/task_tools.task_create`で**担当者が未確定のときだけ**
`services/property_master.find_assignee(content)`で物件名を突き合わせて自動的に補う。
- 突き合わせは表記ゆれに強い設計（正式名称と完全一致でなくても拾う）。ただし**最長一致した
  候補群の担当者が割れる／一致が無ければ何もしない**（自動設定せず未確定のまま＝安全側）。
  「京橋」のような短い接頭辞が別担当の複数物件に当たって誤爆しないよう、短い候補より
  長く具体的に一致した候補を優先する（実装時に発覚したバグの直し方。詳細はSESSION_LOG参照）。
- ローマ字頭字語の略称（例:「CC66」）はJapanese名から自動生成できないため拾えない
  （末尾の数字だけは拾えるので、その数字が台帳内で一意なら結果的に解決できることがある）。
  今後こうした略称が増えたら`property_master.py`にエイリアス辞書を足す想定（現状は未実装）。
- **既存TODOへの遡及適用はしない**（新規に発生するTODOにのみ適用。オーナー確認済み）。
- 台帳の「担当」が更新されたら`python3 ingest_properties.py --no-geocode`を再実行してマスタに反映。
...
- OCR（スキャンPDF）: `python3 ocr_ingest.py`（**二段構え**。下記）。
...
### OCRは二段構え（2026-08-31 変更）

**① macOS Vision → 文字が取れなければ ② claude vision。**

| | macOS Vision | claude vision |
|---|---|---|
| 速さ（実測） | **2.3秒/ページ** | 1件約2分（**186件/2時間**） |
| 費用・枠 | **無料・ネット不要** | 定額枠を消費 |
| 印刷された書類 | ◎ | ◎ |
| **手書き・複雑な帳票・表の構造** | △〜✗ | **◎** |

**なぜ変えたか**: もともと claude だけだったのは「作った時点で他に手段が無かった」だけ。
実測で **7,840件中1,204件（15%）** までしか進んでおらず、残り約6,600件に**35晩以上**かかる計算だった。
さらに 03:00〜05:00 に枠を占有し、2026-08-27 には翻訳ジョブを巻き添えにして全滅させている。

**実測（共有フォルダの未処理スキャン5件）**: 3件が 0.6〜3.8秒で 104〜1,820字を取得して
claude を呼ばずに完了。残り2件は0字で claude へ回る（うち1件は外観パース＝そもそも文字が無い）。

- 実体は `mail-archiver/tools/ocr_pdf`（Swift・macOS Vision）を**借りている**。複製しない
- **無ければ静かに claude へ回る**ので、ビルドしていないPCでも従来どおり動く。
  使うなら各PCで `mail-archiver/tools/build.sh` を1回叩く
- 環境変数 `VISION_OCR_BIN` で場所を変えられる
- ★**claude を無くしたわけではない**。手書きの検針票などは claude の方が正確なので、
  「Vision で読めなかったものだけ claude」という役割分担にしてある
