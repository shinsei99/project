...
| **Excel (.xlsx)** | 先頭に「一覧」シート（日付・氏名・要約・発言数・TODO件数）＋1人1シート。A列=項目 / B列=内容 |
| **Markdown (.md)** | 氏名が `##`、日報の見出しが `###`。1ファイルに全員分 |

Chatwork 投稿用は `chatwork_body()` が別に整形する。**Chatwork は見出し記法が無い**ので
`## 本日の対応` を `【本日の対応】` に、`- ` を `・` に置き換える。
「気づき・注意点（AI所見）」は**既定で投稿に含めない**（フォロー漏れの指摘など上長向けの内容が
入るため）。画面のチェックで含めることもできる。Word/Excel/Markdown には常に入る。
