...
| ファイル | 役割 |
|---|---|
| `db.py` | SQLite のスキーマと検索。FTS5(trigram) で日本語の部分一致検索。**添付の中身も検索対象**（2026-08-31〜） |
| `attach_extract.py` | **添付の中身を索引に入れる**。テキスト層/Office はその場で、スキャンPDF・画像は OCR |
| `ai_search.py` | **試しながら探す検索**。0件でも1件でも止めず、条件を変えて答えまで書く（2026-08-31〜） |
| `tools/ocr_pdf.swift` | OCRの土台（macOS Vision）。`tools/build.sh` で1回ビルドする。**バイナリは git に入れない** |
...
| **00:30** | **メール取込＋英語メールの翻訳100通＋添付の中身＋ベクトル** | 取込7分40秒＋翻訳約15分＋添付①数分＋添付②最大45分 |
| 01:00〜02:00 | （緩衝帯。翻訳が長引いても次とぶつからない） | |
| **02:00〜04:00** | 共有フォルダのスキャンPDFのOCR（AI業務マネージャー側・claude vision） | 1件約2分＝最大60件 |

### 添付の中身を索引に入れる（2026-08-31 追加）

**取込の直後に2段でやる。**「常に全体を文字で把握できる状態」を保つための並びで、順番に意味がある。

| 段 | 対象 | 上限 |
|---|---|---|
| ① 新着 | 直近30日のメールの添付（`--since-days 30`） | **上限なし**。その晩に必ず片付ける |
| ② 積み残し | それより古いもの | `--max-minutes 45`（`ATTACH_MAX_MIN`） |

**分けないと新着が永久に後回しになる。** 積み残し（初回で約4万件）を処理している最中に
時間切れが来ると、その日届いたメールの添付が毎日置いていかれるため。

**OCR は macOS Vision（`tools/ocr_pdf`）＝claude の定額枠を使わない。**
だから 02:00 からの共有フォルダOCRと取り合わない。実測 **2.3秒/ページ・全体で 2.8件/秒**
（4並列。テキスト層あり・スキャン混在の実データ183件で1.1分）。
claude vision だと 186件/2時間＝スキャン11,100件で約60晩かかる計算だった。
...
## 添付の中身の検索（2026-08-31 追加）

**なぜ入れたか**: 「9月2日のスイスホテルの懇親会の詳細」で探しても出てこなかった。
調べたら **会場名はメール本文に一度も出てこず、スキャンPDFの中にしか無かった**。
添付は保管していたが**中身は索引していなかった**ので、どんな語でも当たりようがなかった。

```bash
/usr/bin/python3 attach_extract.py --stats          # 進み具合
/usr/bin/python3 attach_extract.py --since-days 30  # 新着ぶんだけ
/usr/bin/python3 attach_extract.py --max-minutes 45 # 積み残しを小分け
./tools/build.sh                                    # OCRの土台を作る（PCごとに1回）
```

| 形式 | 取り出し方 |
|---|---|
| PDF（テキスト層あり） | pdfplumber |
| PDF（スキャン画像）・jpg/png/heic | **macOS Vision**（`tools/ocr_pdf`） |
| docx / xlsx / xlsm | python-docx / openpyxl |
| **doc / xls（旧Office）** | `textutil`（OS同梱）/ xlrd |
| csv / txt / md / html | そのまま（文字コードは utf-8 → cp932 → euc-jp の順に試す） |

**実測（2026-08-31 の抜き取り60件）**: PDFの **58%はテキスト層あり・42%はスキャン**。

**はまり所**

- **`/usr/bin/python3` で動かす。** pdfplumber/openpyxl/python-docx/xlrd は
  そちらに入っていて、このアプリの `.venv` には**入っていない**
- **文字が取れなくても1行書く**（`method='none'`）。書かないと毎晩同じファイルを試して前に進まない
- **ライブラリ不足を「文字なし」と混同しない**（`method='error'` で残す）。
  混同すると、直したあとも再挑戦されずに黙って抜け落ちる
- **FTS5 は UNIQUE も外部キーも持たない。** 入れ直すときは必ず `DELETE FROM attachment_fts`
  してから入れる（しないと同じ添付が二重に積み上がり、件数がずれる）
- 検索は `JOIN` ではなく `IN(サブクエリ)`。JOIN だと添付が複数当たったメールが二重に数えられる
- **画面に「📎 添付に一致」を出す。** 出さないと、本文を読んでも検索語が見つからず誤検索に見える

## 🤖 AIに探してもらう（2026-08-31 追加）

画面の「🤖 AIに探してもらう」、またはコマンドから:

```bash
/usr/bin/python3 ai_search.py "9月2日のスイスホテルの懇親会の詳細　1ヶ月以内のメール"
```

**なぜ作り直したか**: 従来は **自然文を1回だけJSONに変換 → その条件で1回検索 → 0件なら終了**で、
**結果を見て条件を変える工程が無かった**。実際に上の質問で、
「9月2日」を捨て、「懇親会」と「スイスホテル」をANDの必須語にして0件で終わっていた。

### 緩め方の階段（上から順に試す。**決定的にPython側で**やる）

| # | やること | なぜ |
|---|---|---|
| ① | 表記ゆれを展開してAND | 本文の書き方は `9月2日` / `９月２日` / `9/2` とばらつく |
| ② | 期間を外す | 「1ヶ月以内」の外に本命があることは普通にある（**範囲外にあったと必ず伝える**） |
| ③ | 必須語を1つずつ落とす | 固有名詞2つのANDは0件になりやすい。**珍しい語を残す**（`_rarity` で件数を見る） |
| ④ | 言い換えを足してOR | 懇親会↔懇談会。静的辞書 → 無ければ claude に聞く |
| ⑤ | どれか1語でも含むもの | 最後の網 |

**LLMには3つしかやらせない**（初手の条件・言い換え・答えの文）。claude は60秒で落ちることがあり、
階段までLLM任せにすると検索そのものが死ぬため。

### ★件数で満足しない

**1件見つかっても、それが質問の答えとは限らない。** 実際 2026-08-31 に、①が1件当たったが
それは**同じホテルの別の会（10/6の祝賀会）**だった。そこで、候補を読ませるときに
`answered`（この材料で質問に答えられたか）を返させ、**false なら次の段へ進む**。

実測（同じ質問での動き）:

```
① "スイスホテル" AND "懇親会" AND ("9/2" OR "9月2日" OR "９月２日") 期間あり → 0件
② 期間を外す                                                        → 4件（2024年の別件）→ 合わない
③ 「懇親会」を必須から外す                                          → 5件 → ★本命に到達
→ 添付PDFから 会場・会費・締切まで答えた
```
