...
# ★OCR は二段構え（2026-09-02 オーナー指示）。① macOS Vision（`tools/ocr_pdf`・無料・
#   実測 2.3秒/ページ）→ ② 読めなかったものだけ claude vision。**枠を使うのは②だけ**。
#   節約モード中は②を呼ばず「後日に回す」（行を書かないので次の晩に再挑戦する）＝
#   この工程自体をスキップする必要はない。03:00 の AI業務マネージャーのOCRとは時間帯をずらしてある。
