...
### [x] 添付のOCRを二段構えにする（2026-09-02・オーナー指示）

Vision で読めなかった添付だけ claude vision へ回す。節約モード中と claude 失敗時は
**行を書かずに後日へ回す**（`none` で確定させると二度と読まれないため）。詳細は `SESSION_LOG.md` 冒頭。

### [ ] 既存の `none` 1,849件を claude で拾い直すか決める（★オーナー判断が要る）

二段構えの**前**に「Vision で読めなかった」と記録された分。`--retry-empty` を一度流せば
2段目が拾えるが、**1,849件ぶんの定額枠を使う**。夜間ジョブは新着から進むので、
放置するとこの山は拾われないまま残る。
