...
**いまやることは無い。審査の結果を待つだけ。**
`python3 appstore_api.py --review com.shinsei.shosai` で見る。通ったら
`CLAUDE.md` と直下 `TODO.md` を「1.0.1 が配信中」へ書き換える。
**アイコンが App Store と端末で新しくなるのは配信されてから**（審査中は旧アイコンで正常）。
出し直すことになったら**必ず build 3 へ**（`./ios-build-guard.sh digital-shosai --bump`）。
