# -*- coding: utf-8 -*-
"""PDFの向き（縦横・回転）を解析前に自動補正する共有ユーティリティ — **直下に1本だけ置く**。

2026-08-23: これまで8つのアプリに**同じ中身のコピー**が置かれていた（全部同一ハッシュ）。
直すたびに8箇所を触ることになり、1箇所忘れれば「あのアプリだけ向きが直らない」になる。
`japanpost_api.py` などと同じく直下へ集約し、各アプリには**委譲だけのファイル**を残した
（アプリ側の `from pdf_orient import ensure_upright_pdf` は今までどおり動く）。
