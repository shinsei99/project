...
## ✅ 送信・再提出まで完了（2026-08-29）

| 時刻 | 内容 |
|---|---|
| 09:10 | Resolution Center に**返信**（下の英文をそのまま） |
| 09:12 | **審査へ提出**（提出ID `684ea528-c1aa-40b7-9a08-d3dfa13a9e63`） |

**API で確認**: `1.0.5 = WAITING_FOR_REVIEW` ／ `1.0.4 = READY_FOR_SALE`（配信中の版は無事）。
審査ノート899字も入った状態で出ている。

**★順番を守れた**（返信 → 2分後に提出）。8/28 の KeyTag は「返信なしで再提出、説明は
審査ノートだけ」で同じ定型文を返されている。**説明が先に並んでいる状態で列に入れる**のが要点。

**次にやること**: 結果を待つだけ。`python3 ../appstore_api.py --review com.shinsei99.scrapmemo`。
**★通るまで KeyTag を再提出しない**（返信の中で「1件ずつ出す」と書いたため。
同時に出したことが今回の引き金だった）。
