...
def build_succession(d):
    ...
    hosho = d.get("hosho_name") or otsu  # 新連帯保証人は旧賃借人(乙)が就くのが定型
    preamble = (
        f"賃貸人　{ko}（甲という）と賃借人　{otsu}（乙という）及び{hei}（丙という）は、"
        f"甲・乙間において締結した、{d['orig_date']}付の頭書物件の賃貸借契約"
        "（以下原契約という）の契約上の地位承継に関して以下のとおり覚書を締結する。"
    )
    body = [
        {"text": f"（地位承継の期日）\n　　乙（{otsu}）は{d['succ_date']}（以下承継期日という）をもって原契約の"
                 f"賃借人としての地位を丙（{hei}）に承継させるものとし、甲（{ko}）はこれを承諾する。"},
        {"text": f"（契約当事者の読み替え）\n　　甲（{ko}）・丙（{hei}）は、承継期日以降、原契約上の賃借人乙（{otsu}）を"
                 f"丙（{hei}）と読み替え、原契約を甲（{ko}）・丙（{hei}）間に適用する。"},
        {"text": f"（契約書の引継ぎ）\n　　丙（{hei}）は、原契約の原本を乙（{otsu}）より引き継いだ。"},
        {"text": f"（権利義務の承継）\n　　丙（{hei}）は承継期日をもって、原契約に基づき乙（{otsu}）が甲（{ko}）に対して有する、"
                 f"権利または負担する一切の義務を乙（{otsu}）より承継する。"},
        {"text": f"（確認条項）\n　　原契約上の地位の承継に関し乙（{otsu}）・丙（{hei}）間に紛争が発生した場合は、"
                 f"乙（{otsu}）及び丙（{hei}）の全責任と費用負担においてこれを処理解決することとする。"},
        {"text": f"（連帯保証人）\n　　地位継承に伴い{hosho}は新連帯保証人となり、原契約に基づき丙（{hei}）が将来"
                 f"甲（{ko}）に対して負担する一切の債務につき丙（{hei}）と連帯して保証し、債務の履行の責に任ずる。"
                 f"甲（{ko}）はこれを承認する。"},
