...
>
> **App 記録の新規作成・アップロード・提出をしないこと。** ipa やスクショを作るところまでは可
> （外に出なければ問題ない）。
>
> 2026-08-29 に **3本まとめて 4.3(a)（スパム）でリジェクト**された。API で実測:
>
> | アプリ | 状態 |
> |---|---|
> | KeyTag鍵管理 1.0 | リジェクト（要対応） |
> | スクラップメモpetapeta 1.0.5 | リジェクト（要対応） |
> | にゃんこのアイス屋さん 1.0 | リジェクト（要対応） |
>
> **アプリ個別ではなく、アカウント全体の出し方を見られている**と考えるのが自然:
> スクラップメモ 1.0.5 は**配信中アプリのUI修正だけの更新**なのに 4.3(a)／KeyTag は
> 1,153字の審査ノートで具体的に反論済みだったのに同じ定型文／6月末からの2か月で **9本**提出し、
> 多くが Capacitor の同じ殻（ネイティブ実行ファイルがほぼ同一バイト数）。
>
> **この状況でカラー・グラビティを出すのは最悪手。** 4.3 を繰り返すと
> アプリ削除・アカウント停止まで行く。
>
> オーナー判断: **まず KeyTag だけ Resolution Center に返信して様子を見る。**
> 結果が出るまで他は動かさない。再開するときはこの節を消してから。

**2026-08-28 時点の状態: アーカイブとipaの書き出しまで完了**（＝下の手順は**再開が決まってから**使う）。
