...
#   にゃんこアイスは Capacitor 6＝CocoaPods なので -workspace が要る。アプリごとに違う。
#
# 使い方:
#   ./screenshots/shoot.sh title aim flight clear stages           # iPhone
#   DEVICE=ipad ./screenshots/shoot.sh title aim flight clear stages
set -euo pipefail
cd /Users/apple/color-gravity
...
BUNDLE=com.shinsei99.colorgravity
...
for SHOT in "$@"; do
  echo "── $SHOT ($DEVICE) ──"
  # www/ の正本に、撮影用の細工を「ゲームの起動直前」へ差し込む
  python3 - "$SHOT" <<'PY'
import sys
shot = sys.argv[1]
...
assert src.count(marker) == 1, "差し込み位置が特定できない（起動の書き方が変わった？）"
...
PY
  ...
  xcodebuild -project ios/App/App.xcodeproj -scheme App -configuration Debug \
    ...
    || { echo "  ❌ ビルド失敗（$SP/shot-build.log）"; tail -20 "$SP/shot-build.log"; exit 1; }
