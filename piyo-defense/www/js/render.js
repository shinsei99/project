...
function drawGround(stage) {
  ...
  var si = Math.max(0, Math.min(19, (stage||1) - 1));
  var bg = _SBG[si];
  // ★2026-08-29: 草原を 108 → **40px** に詰めた（オーナー案「草原部分を縮めればいい」）。
  //   HUDと仲間ボタンを盤の外に出したぶんと合わせて、canvas がちょうど 9:16 になる。
  //   **空（＝敵が落ちてくる距離）は654pxのまま**なので、落下時間も難易度も変わらない。
  var HL = _H - 40;                        // 手前の草地の稜線
