...
~/Library/CloudStorage/Dropbox-個人/CLAUDE/
  ...
  ├ 一次資料/               … 国交省・国税庁・個情委のガイドラインと RETIO 判例（194件・215MB）
  │                          AI業務マネージャーの「法令」棚の元。取り込みは bookshelf/ingest_all.py
  ├ 書籍/                   … 蔵書のスキャンPDF（78件・5.3GB）。同じく「本」棚の元
...
**印は `~/.ai-quota-saver` の1ファイルだけ**（1行目が期限）。夜間ジョブが起動時に見て工程を飛ばす。

**★期限つきにしてある**（Zennの静止期間と同じ作り）。`on/off` だけだと**戻し忘れて永久に止まる**。
日時を過ぎれば**勝手に元へ戻る**。**launchd は外さない**（外したら戻し忘れる）。

現在の設定: **OFF（通常運転）**。2026-09-02 に解除した（週枠が 0% に戻り、9/13 まで週上限
+50% のキャンペーンもあるため、期限の 9/4 6:00 を待たず前倒しで解除）。
...
#### ★節約モード中は Fable を使わない（2026-09-01 オーナー指示）

枠のゲージは**入れ子**で、「全モデル」と「Fableのみ」は**どちらか**ではない。
Fable を使うと**両方**が減る。しかも Fable は最上位モデルで、1トークンあたりの重さは
**Opus のちょうど倍**（API価格 $10/$50 対 $5/$25。Sonnet 比では約3.3倍）。
つまり「Fableのみが 5% だから余っている」ではなく、**Fable を使うと全モデル側がいちばん速く減る**。

- **対話（この会話）でも `/model` で Fable に切り替えない**。節約中は Sonnet 、
  込み入った不具合調査のときだけ Opus に戻す
- **スクリプトに `--model fable` を足さない**。自動処理は全部 `sonnet` に固定してある
  （AI業務マネージャーの `DEFAULT_MODEL="sonnet"` ／ メールの `ai_search.py`・`ai_query.py`）
- 忘れても気づけるように、**`./ai-quota-saver.sh` の状態表示が Fable の指定を探して警告する**
  （節約中に `--model fable` 等がリポジトリにあれば赤字で出す）
...
| 置き場 | 中身 |
|---|---|
| Dropbox `共有フォルダ/（★必読★）新共有フォルダ/社内ツール/` | 各アプリの `.url`（**23本**。2026-09-03に AI受付＆起票カウンター を撤去）＋ `icons/*.ico` |
| その**1つ上**（`（★必読★）新共有フォルダ/` 直下） | **`業務マニュアル.url` の1本だけ**（2026-09-03に `横断ファイル検索.url` を撤去）。全社員が毎日使う入口なので浅い位置に置く |
| `Desktop/社内ツール/`（このMacのみ） | `.app`（**30本**。2026-09-03に2本撤去） |

> **★2026-09-03: 廃止した2本の入口を撤去した**（`横断ファイル検索` / `AI受付＆起票カウンター`）。
> Dropbox の `.url` 2本・`icons/*.ico` 2個・Desktop の `.app` 2本。
> 退避先は `~/Desktop/削除予定_20260903/_旧ショートカット/`。
>
> **★社員への案内がまだ出ていない。** リンクが消えたので、
> 「**ファイル探しと不具合報告は Chatwork の claude に頼んでください**」を伝える必要がある。
> **`全ファイル一覧.xlsx` 本体は共有フォルダに残してある**（社員が直接開くため。週次更新も継続）。
