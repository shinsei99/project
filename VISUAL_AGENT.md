...
### ★ `shot` にURLを渡さない（古い画面が写る・2026-08-24 実際にやった）

`./va.sh shot <名前>` の引数は**保存するファイル名**であって URL ではない。
URL を渡しても**遷移せず**、開きっぱなしの前のタブをそのまま撮る。**エラーは出ない**ので、
「直したのに画面が変わらない」「もう直っているのに古い警告が写る」という形で気付く。

```bash
./va.sh goto http://127.0.0.1:8536      # ← 開くのはこっち
./va.sh shot jyuusetsu-top              # ← 撮るだけ。名前を渡す
```

実際にこれで、**すでに解決済みの警告**を現在の不具合として報告してしまった（AI重説 8536）。
同じ画像が何度撮っても**バイト数まで同じ**なら、まず遷移していないことを疑う。
