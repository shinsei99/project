...
| digital-shosai | **メイン** | **2026-08-27: 1.0 は審査を通って配信中**（API で実測。8/24時点の「審査待ち」から変わっていた）。**同日、アイコンをオーナー支給の画像に差し替え、1.0.1 / build 2 を Archive まで済ませた**（`~/Library/Developer/Xcode/Archives/2026-08-27/デジタル書斎 2026-08-27 1.0.1-build2.xcarchive`・22MB。中身が 1.0.1/build2 で iPhone/iPad とも新アイコンになっていることを確認済み）。支給画像は白背景に角丸アイコンが乗った形で**そのまま使うとホーム画面で角が白く欠ける**ため、`icon-src/make_icon.py` を「切り出す→1024へ→角の外を地色で塗る」処理に書き換えた（原本 `icon-src/source_2026-08-27.png` も git に入れた）。**同日、アップロードまで完了＝ASC で `build 2 … VALID` を確認**（Organizer を使わず `xcodebuild -exportArchive` → `xcrun altool --upload-app` の3コマンドで通した。手順とつまずき所は `digital-shosai/HANDOFF-APPSTORE.md` 冒頭。※`altool` は `--apiKey` にパスを取らず `~/.appstoreconnect/private_keys/` しか見ないので鍵をそこへ置いた）。**★次はオーナーが手で: ASC で 1.0.1 を作り build 2 を選んで審査へ提出**（新機能欄は「アプリアイコンを新しくしました。」）。残る積み残しは**実機（iPhone）で1度通すこと**。手順は `digital-shosai/HANDOFF-APPSTORE.md` 冒頭 | 2026-08-27 |
