...
> ### ★人にしかできないこと（残り2件）
>
> 1. **スクリーンショットの撮り直し**。`simtap.py` が使えなかった（**Simulator のウインドウが
>    0枚**で操作できない。原因未特定）。**実機で撮るのが早い**。並びは `store-text.md` に
>    決めてある: ①かざして鍵が特定された画面 ②台帳 ③貸出 ④タグに書き込む ⑤サーバー連携
> 2. **ASC へのメタデータ入力と提出**。文言は `store-text.md` からそのまま貼れる
>
> ### 判断が要ること
>
