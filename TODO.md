...
| mail-archiver | サブ | **新規（2026-08-20）: メールアーカイバ**（IMAP容量対策・8535・127.0.0.1）。取り込み／サーバー側削除（14日ルール＋SHA256・UIDVALIDITY・Message-IDの照合＋UID EXPUNGE）／閲覧UI／偽IMAPでの通し検証（30項目）まで実装済み。**このPCのMail.app経由で実メール19通(4.0MB・添付2件)を取り込み、`--verify` 問題0件・日本語全文検索・画面まで確認済み。** ただし**IMAP経由は未実行**（iCloudはApp用パスワードが要る。`.env` は用意済み・削除は無効のまま）。次は ①App用パスワードを発行して `--since-days 7 --limit 20` ②`restore.py`（.emlをAPPENDで戻す） | 2026-08-20 |
...
- **★ここから: 「APIの取得と整理」の再開点は `API_STATUS.md`（直下）。2026-08-20 に更新済み。**
  ...
  **保有APIの一覧説明書を作成** … 直下 `API一覧説明書.xlsx`（保有29件／取りに行くリスト18件／決まり9件）。
