...
| maisoku-converter | サブ | **マイソクコンバーター**（8505・社内LAN共有あり）。**2026-08-21: 帯変えモードが縦向き（A4縦）のマイソクに対応**。画像の縦横比から用紙の向きを自動判定し、画像枠の行高・自社帯の高さ・帯の文字サイズまで合わせる（A4縦のマイソクが紙幅の約70%→100%に。正方形も横181mm→縦202mm）。`smoke_test.py` を新設（縦/横/正方形の3通り）。実物 `物件概要書_加東市.pdf`（A4縦）で画面から生成まで確認。積み残し: ①帯の建設業免許番号が**横向きのとき**担当者欄へはみ出す（以前からの症状・未修正）②通常モード（AI解析）の切り抜きにも枠エディタを使う | 2026-08-21 |
