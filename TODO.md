> ## 🖥 2026-08-24（月）メインPCで最初にやること — 業務日報の本番化
>
> **サブPCで「業務日報」機能を作り終えた（2026-08-21・push済み）。残りはメインPCの2作業だけ。**
>
> ```bash
...
> ```
>
> 1. **上のpullとworker再起動。** サブPCにworkerは常駐していないので、
>    **毎日18:30の日報の自動処理はメインPCのworkerでしか動かない。**
> 2. **`/bin/bash` にフルディスクアクセスを与える**（システム設定＞プライバシーとセキュリティ）。
>    launchd の常時起動プロセスは CloudStorage を読み書きできず、次の2つが失敗する:
>    - 保管先 Dropbox『共有フォルダ/（★必読★）新共有フォルダ/社内・総務/業務日報』
>    - 休暇スケジュール GoogleDrive『…/ルーティーン/年間休暇スケジュール2026.xlsx』
>    （`shorui-cabinet` 8528 で同じ対処を実施済み）
> 3. できれば**初回の本番投稿を人が見ている時間に一度手で流して確認**する。
>
> **できていること**（2026-08-21 サブPCで実測・投稿確認済み）:
> Chatworkの会話＋claude宛の報告 → 社員1人ずつの日報（3項目・時刻なしの要点・社内の人の名前は
> 書かない）→ Excel1シートに 塚本→松本→森 → 18:30に保管＋Chatworkへアップ →
> 会社の休業日（年間休暇スケジュールのオレンジ・2026年106日）は作らない。
> **オーナーの個人Chatworkとグループの両方へ実投稿して見え方を確認済み。**
> 仕様・はまり所は `chatwork-ai-manager/README.md`（SESSION_LOG/TODOはgitignoreで届かない）。
...
>
> **★注意**: `maisoku-converter/app.py` に**私の作業ではない未コミット変更が193行**ある
> （14:48時点・縦マイソク対応）。別セッションの作業と思われるので触っていない。
...
| ↑ chatwork-ai-manager 業務日報 | **メイン** | **2026-08-21（サブPCで実装）: 「業務日報」機能を追加した**（オーナー依頼）。その日のChatwork会話＋TODOの動きから、社員1人ずつの日報をAIが書く。管理画面に「📝 業務日報」タブ（10画面目）。**運用開始は来週から**（オーナー指示）。**Chatworkへは自動投稿しない**（`outbox.NEVER_AUTO_KINDS` を新設。post_mode=auto でも日報だけは必ず人が承認）。本人判定は account_id のみ（社員の「森」さんと入居者の「森様」が同じ会話に居るため名前判定は不可）。8/21・8/20 の実データで 塚本・松本・森 の日報を生成し、画面も `./va.sh` で目視・崩れ0を確認済み。**★メインPCで一度回すこと**: サブPCのDBは tasks が3件しか無く、日報の「本日動いたTODO／未完了TODO」欄だけ実測できていない。仕様と設計理由は `chatwork-ai-manager/README.md` の「業務日報の自動作成」節（SESSION_LOG/TODOはgitignoreで届かないため） | 2026-08-21 |
| jyuusetsu-research | サブ | **AI重説調査〜Excel自動入力**（開発中・launchd未割当）。**2026-08-20: 用途地域が常に空だったのを修正**（`XKT001`→**`XKT002`**＋プロパティ名・単位・最寄り代用の100m上限）。ジオコーディングを Google 併用に（`location_type` が ROOFTOP のときだけ採用。加東市で Google が892m外した実測あり）。ストリートビューの節を追加。**e-Statの人口・世帯数も実装済み**（統計表0000020101／A1101・A7101／市区町村コードは国土地理院）。**次は ①Webキーのリファラ制限で社内画面が403 → Console で「Maps Embed だけに制限したキー」を作り `.env.google-maps` に `GOOGLE_MAPS_EMBED_KEY=` を追記（人の作業。コードは対応済み）** | 2026-08-20 **★2026-08-21: 「基本4種（賃貸重説/売買重説/賃貸契約/売買契約）をできるだけ自動作成」へ方針拡大＋3アプリ統合を決裁（着手前）。****実測: 調査で25項目取っているのに書式へ流し込んでいるのは8項目だけ**（賃貸重説にいたっては3項目）。法令制限・災害・周辺環境・価格指標の12項目は取得済みなのに未使用＝ここを埋めるのが最短。**人にお願い: 賃貸契約書と売買重説（土地戸建）の白紙Excel書式が手元に無い**（templates/*.xlsx はgitignore）。 このアプリを母体に`legal-crosscheck`（4点クロスチェック）を④タブとして吸収し、`tokuyaku-generator`の特約機能を③タブとして載せる（**8513は恒久的に残す**＝置き換えではない。170項目カタログはコピーせず共有）。取引種別で③④を出し分け。port は 8536 を予約。**統合で消えるのは legal-crosscheck の1本だけ（53本→52本）**。様式つきの計画は `jyuusetsu-research/TODO.md` の先頭 | 2026-08-21 **2026-08-21: legal-crosscheck を完全に吸収し、アプリを削除した（53本→52本）。**④クロスチェック（重説・契約書PDF→4者照合→検閲報告書Excel）と、Web法令調査（防火地域・高度地区・日影規制の補完）を画面に載せた。公式書式186本の流し込みも実装済み | 2026-08-21 |
...
| maisoku-converter | サブ | **マイソクコンバーター**（8505・社内LAN共有あり）。**2026-08-21: 帯変えモードを縦（A4縦）マイソクに対応**。①画像の縦横比から用紙の向きを自動判定 ②A4縦は本体幅195mm固定・中央寄せ ③白フチ自動カット（既定オン）④マイソクと帯の間にすき間4mm ⑤帯の文字を実寸で揃える。**途中で見つけた重大バグ: 列幅→ptの換算が2割狂っており、貼った画像が横に1.2倍伸びていた**（TwoCellAnchor→OneCellAnchor＋実測換算 `_PT_PER_CHAR=6.0` で解消。**Excelで実測確認済み**：帯1150pt vs 画像1149.75pt）。`smoke_test.py` 新設（全項目OK）。**次: 実際に1枚印刷して既定値でよいか確認／通常モード（AI解析）は未対応のままA4横** | 2026-08-21 |
