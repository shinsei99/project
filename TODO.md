...
| ↑ chatwork-ai-manager 業務日報 | **メイン** | **2026-08-21（サブPCで実装）: 「業務日報」機能を追加した**（オーナー依頼）。その日のChatwork会話＋TODOの動きから、社員1人ずつの日報をAIが書く。管理画面に「📝 業務日報」タブ（10画面目）。**テストは 2026-08-21 に完了。来週から本番運用**（オーナー指示）。**18:30の自動処理は Excel を Chatwork へ自動アップする**（`post_file` を直接呼ぶ＝outboxを通らず post_mode も見ない。オーナーの明示指示。止めるなら設定 `daily_report_upload=0`）。承認が要るのは**日報の本文をメッセージとして投稿する経路だけ**（`outbox.NEVER_AUTO_KINDS`。post_mode=auto でも人が承認）。本人判定は account_id のみ（社員の「森」さんと入居者の「森様」が同じ会話に居るため名前判定は不可）。8/21・8/20 の実データで 塚本・松本・森 の日報を生成し、画面も `./va.sh` で目視・崩れ0を確認済み。**★メインPCで一度回すこと**: サブPCのDBは tasks が3件しか無く、日報の「本日動いたTODO／未完了TODO」欄だけ実測できていない。仕様と設計理由は `chatwork-ai-manager/README.md` の「業務日報の自動作成」節（SESSION_LOG/TODOはgitignoreで届かないため） | 2026-08-21 |
