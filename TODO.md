...
|---|---|---|---|
| photo-remake | サブ | **フォトリメイク（iOS）。2026-08-25: 図形注釈を新設（12種＝四角/角丸/丸/三角/ひし形/五角形/六角形/星/ハート/吹き出し/十字/バツ）**。塗りの色と枠線の色を別々に指定でき、それぞれ「なし」にできる（片方を消すと必ずもう片方が残る）。移動/大きさ/回転に対応し、書き出しはプレビューと同じパスで焼き込む。**あわせて上部「写真」ボタンの誤タップ対策**（編集内容があるときだけ確認ダイアログ）。Debug/Release ともビルド通過、シミュレータで画面と書き出しPNGを目視確認済み。**版を 1.1.0/build4 へ繰り上げ済み（`ios-build-guard.sh` で衝突なし）だが App Store へは未提出＝提出は人の判断**。※`.sheet` を複数積むと出ないので `ActiveSheet` の1本に統一した（README に記録） | 2026-08-25 |
