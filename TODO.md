...
> | 2 | ~~**にゃんこアイス Web版(gh-pages)への反映**~~ → **2026-08-28 に「やらない」と決着**（下記） |
> | 3 | ~~**スクラップメモ Web版(gh-pages)への反映**~~ → **2026-08-28 に「やらない」と決着**（下記） |
>
> **★GitHub Pages との付き合い方（2026-08-28 オーナー判断・全アプリ共通）**
>
> **触れる Web版（`index.html`）はもう更新しない。ただしフォルダは絶対に消さない。**
> 同じフォルダに **Apple が必須にしているサポートURL・プライバシーポリシーURL** が同居している。
> スクラップメモ／ママカウンター／不動産電卓／ネオンブロックの4本は、登録済みの必須URLが
> **アプリのフォルダの中の `support.html` / `privacy.html`** を指している（API で実測）。
> 消すと審査に出せなくなり、利用者の問い合わせ先も切れる。
> **新規にApp Storeへ出すアプリは、提出前にこの2ページを作ること**（にゃんこアイスは両方とも未登録）。
...
| nyanko-ice | **メイン** | **にゃんこアイス（ゲーム・App Store 未提出）。2026-08-27: 見た目を「アイス屋の店内」に作り替え、書体と音を入れた**（オーナー依頼「フリー素材とか活用してもっと魅力的に」）。空白だった Y=280〜580 を埋め、画面を4層（木の看板／明るい店内／冷凍ショーケース／カウンター）に再構成。**Zen Maru Gothic（SIL OFL）を使用文字だけに絞って同梱**（`tools/fetch-font.py`・32KB×2）。ネコ店長に体・エプロン・コック帽を付けて前足をケースの縁へ。**音はオシレータ直打ちを全廃し、効果音10種＋BGMループ＋オン/オフボタンに**（**ファイルは同梱せず WebAudio で合成**。差し替えたいときは `www/assets/audio/` に置いて `USE_FILES=true` にするだけ）。Console エラー0件・3画面を目視。**★残り: ①音をオーナーが一度聴く（こちらでは聴けない） ②提出するなら先に `support.html` / `privacy.html` を作る（Apple必須。このアプリは両方とも未登録＝2026-08-28 に API で確認）。Web版(gh-pages)の更新は 2026-08-28 に「やらない」と決着** | 2026-08-28 |
