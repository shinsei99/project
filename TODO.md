...
| nyanko-ice | **メイン** | **にゃんこアイス（ゲーム・App Store 未提出）。2026-08-27: 見た目を「アイス屋の店内」に作り替え、書体と音を入れた**（オーナー依頼「フリー素材とか活用してもっと魅力的に」）。空白だった Y=280〜580 を埋め、画面を4層（木の看板／明るい店内／冷凍ショーケース／カウンター）に再構成。**Zen Maru Gothic（SIL OFL）を使用文字だけに絞って同梱**（`tools/fetch-font.py`・32KB×2）。ネコ店長に体・エプロン・コック帽を付けて前足をケースの縁へ。**音はオシレータ直打ちを全廃し、効果音10種＋BGMループ＋オン/オフボタンに**（**ファイルは同梱せず WebAudio で合成**。差し替えたいときは `www/assets/audio/` に置いて `USE_FILES=true` にするだけ）。Console エラー0件・3画面を目視。**2026-08-28（メインPC・午後）: 広告を全部外し、App Store の提出直前まで用意した**（オーナー判断「一旦、広告なしで出す」）。AdMob の CSS/DOM/JS・プラグイン・Info.plist の2キー・UMP の Pod まで削除し、**ipa の Frameworks が Capacitor と Cordova だけ**であることを実測。ゲームオーバーの「動画を見てコンテニュー」は動画なしの `▶ つづきから` に。**iPadの余白も解消**（`max-width:470px` → `min(100vw,62vh)/720px`。iPhone の見え方は不変を前後比較で確認）。`www/support.html`・`www/privacy.html` を新設して公開（200を実測）。ストア文言（`store-text.md`）・**スクショ iPhone 5枚 + iPad 5枚**・**1.0 / build 2** まで ASC に投入済み＝**提出準備中**。Web版(gh-pages)も同日反映済み（`DEPLOY_FOLDERS` に `nyanko-ice:www` を追加＝以後 push で自動）。**★人がやること3つ: ①音を一度聴く（公開版で聴ける） ②ASCの画面で 価格(無料)・App のプライバシー(データを収集しません)・年齢制限(4+) を設定 ③審査へ提出** | 2026-08-28 |
