...
| jyuusetsu-research | サブ | **AI重説アシスタント**（開発中・8536予約）。**2026-08-23: 災害3項目を実装**（洪水 XKT026／土砂 XKT029／津波 XKT028。コード値は公式コードリストで裏取り。「区域外」と「判定不可」を分ける）。**書式の災害欄は□のチェックで、しかも3項目とも「外」側を指していた不具合を修正**（テキストを流し込まず、**土砂災害の「内」だけ自動で■**。津波は制度が別・洪水欄は地図添付のため触らない）。**防火地域が XKT014 で取れると判明**（「入手経路が無い」を解消。高度地区は依然不可）。**抵当権を土地／建物に分離**（土地の抵当権が建物欄に入っていた件）。**8517 の薄い入口が壊れていたのを修正**（`_combine_shubetsu` が `import *` で運ばれず smoke_test が落ちていた）。**次: ①4書式の印刷イメージを目視（このPCは LibreOffice 無しで不可・メインPC向き）②抵当権を実物の謄本PDFで確認（共有パーサのスキーマを変えたため）** | 2026-08-23 |
