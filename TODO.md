...
|---|---|---|---|
| photo-remake | サブ | **フォトリメイク（iOS）。2026-08-25: 図形注釈を新設し、矢印もその中に統合（全13種＝矢印/四角/角丸/丸/三角/ひし形/五角形/六角形/星/ハート/吹き出し/十字/バツ）**。下部パレットは「文字・図形・モザイク・調整・切り抜き」の5つ（矢印ボタンは図形に吸収）。**「種類」タブで矢印⇄図形を入れ替えられる**（中点・長さ・向きを引き継ぎ Undo 可）。塗りの色と枠線の色は別々に指定でき、それぞれ「なし」にできる。**あわせて上部「写真」ボタンの誤タップ対策**（編集内容があるときだけ確認ダイアログ）。Debug/Release ともビルド通過、シミュレータで画面と書き出しPNGを目視確認済み。**版を 1.1.0/build4 へ繰り上げ済み（ios-build-guard.sh で衝突なし）だが App Store へは未提出＝提出は人の判断**。※.sheet を複数積むと出ないので ActiveSheet の1本に統一した（README に記録） | 2026-08-25 |
