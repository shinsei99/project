...
> | 2 | **にゃんこアイス Web版(gh-pages)への反映**。★`index.html` だけでなく **`www/assets/fonts/` も一緒に置く**こと（無いと文字が全部□になる） |
> | 3 | **スクラップメモ Web版(gh-pages)への反映** |
> | 4 | ~~**スクラップメモ iOS**~~ → **2026-08-28 に実施済み。1.0.5 / build 9 をアップロードし ASC は「提出準備中」＝あとは審査へ提出を押すだけ** |
>
...
| nyanko-ice | **メイン** | **にゃんこアイス（ゲーム・App Store 未提出）。2026-08-27: 見た目を「アイス屋の店内」に作り替え、書体と音を入れた**（オーナー依頼「フリー素材とか活用してもっと魅力的に」）。空白だった Y=280〜580 を埋め、画面を4層（木の看板／明るい店内／冷凍ショーケース／カウンター）に再構成。**Zen Maru Gothic（SIL OFL）を使用文字だけに絞って同梱**（`tools/fetch-font.py`・32KB×2）。ネコ店長に体・エプロン・コック帽を付けて前足をケースの縁へ。**音はオシレータ直打ちを全廃し、効果音10種＋BGMループ＋オン/オフボタンに**（**ファイルは同梱せず WebAudio で合成**。差し替えたいときは `www/assets/audio/` に置いて `USE_FILES=true` にするだけ）。Console エラー0件・3画面を目視。**★残り2件: ①音をオーナーが一度聴く（こちらでは聴けない） ②Web版(gh-pages)の更新＝外部公開なので指示待ち。更新するときは `www/assets/fonts/` も一緒に置くこと** | 2026-08-27 |
