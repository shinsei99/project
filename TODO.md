...
| maisoku-converter | サブ | **マイソクコンバーター**（8505・社内LAN共有あり）。**2026-08-20: 帯変えモードで、プレビュー上の切り取り枠をドラッグで移動・サイズ変更できるようにした**（新規 `crop_component/`。スライダー4本と双方向同期・リセット付き）。検証は合成テスト画像で Playwright により実施（枠⇄スライダー・リセット・Excel生成まで／Console エラー0件）。次は ①実物の他社マイソクPDFで一度通す ②通常モード（AI解析）の切り抜きにも同じ枠エディタを使う | 2026-08-20 |
