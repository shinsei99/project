...
| chatwork-ai-manager | Chatwork/LINE常駐AIエージェント（社内RAG・TODO/案件・Web/国交省API）。**常駐4サービスはメインPCで稼働中／サブPCは引き継ぎ受領済みで画面8540のみ起動**（worker・ngrokは1台のみ・同時起動禁止）。**2026-08-17: アプリ開発能力（DEVELOPMENT Agent）＋共通Visual Agent＋中断復元を追加し本番反映済み**。次は①LINEから実地で開発依頼を1本通す ②検証用に作った `todo-app` の要否判断 ③週次TODO棚卸し（金18時・月10時）の実装 | 2026-08-17 |
