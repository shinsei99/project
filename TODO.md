...
| jyuusetsu-research | サブ | **AI重説アシスタント**（開発中・8536予約）。**2026-08-21（夜）: 残っていた完了条件「4書式の目視」を実施し、バグを2件見つけて修正**（賃貸重説で**物件の所在地がハザード欄に入っていた**／1セルに2項目の二重割り当て）。**住所の扱いを作り直した**: 謄本に住居表示は無いので `所在地`（住居表示）と `登記所在`（謄本）を分離。入力欄を「町名まで」＋「丁目・番・号」に割り、**日本郵便で町名まで検証**（丁目以降は確認不可＝人が入力）。入力順を**マイソク→謄本→住所**に組み替え、分かる部分は自動表示。**マイソク解析を新設**（解析のやり方だけ移植。8505 には触っていない）。**書式を複数まとめて作れる**ようにした（＋で何本でも／2本以上はZIP）。**次: ①印刷イメージの目視（QuickLookは1ページ目のみ・ExcelのAppleScript PDF化は-50で不可）②災害3項目は `hazard_service` が空を返すスタブのままで未実装**（chatwork-ai-manager の `gis_land_info` に実装あり） | 2026-08-21 |
