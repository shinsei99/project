...
> | 4 | **スクラップメモ iOS**（配信中の 1.0.4/build 8 はそのまま）。載せるなら `npm run sync`（`npx cap sync` 単体は不可）→ `./ios-build-guard.sh scrapmemo-petapeta --bump` → Archive で build 9 |
>
