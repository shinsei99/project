...
| jyuusetsu-research | サブ | **AI重説調査〜Excel自動入力**（開発中・launchd未割当）。**2026-08-20: 用途地域が常に空だったのを修正**（`XKT001`→**`XKT002`**＋プロパティ名・単位・最寄り代用の100m上限）。ジオコーディングを Google 併用に（`location_type` が ROOFTOP のときだけ採用。加東市で Google が892m外した実測あり）。ストリートビューの節を追加。**次は ①Webキーのリファラ制限で社内画面が403 → Console でEmbed専用キーを作る（人の作業）②e-Statの実装（appId登録待ち）** | 2026-08-20 |
| mail-archiver | サブ | **新規（2026-08-20）: メールアーカイバ**（IMAP容量対策・ローカル保管＋全文検索・8535・127.0.0.1）。取り込み／サーバー側削除（14日ルール＋SHA256・UIDVALIDITY・Message-IDの照合＋UID EXPUNGE）／閲覧UI／偽IMAPでの通し検証まで実装済み。**本番アカウント（shin@daikyocorp.co.jp）へはまだ一度も接続していない＝要オーナー確認。**次は ①`--since-days 7 --limit 20` で小さく初回取り込み ②実測を README に記録 ③`restore.py`（.emlをAPPENDで戻す） | 2026-08-20 |
...
- **★ここから: 「APIの取得と整理」の再開点は `API_STATUS.md`（直下）。2026-08-20 に更新済み。**
  ...
  **残りの人の作業**: 国税庁 法人番号（発行2週〜1か月・最優先）／e-Stat の appId 登録／
