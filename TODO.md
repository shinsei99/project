...
| nyanko-ice | **メイン** | **にゃんこアイス（ゲーム・App Store 未提出）。2026-08-27: 見た目を「アイス屋の店内」に作り替え、書体と音を入れた**（オーナー依頼「フリー素材とか活用してもっと魅力的に」）。空白だった Y=280〜580 を埋め、画面を4層（木の看板／明るい店内／冷凍ショーケース／カウンター）に再構成。**Zen Maru Gothic（SIL OFL）を使用文字だけに絞って同梱**（`tools/fetch-font.py`・32KB×2）。ネコ店長に体・エプロン・コック帽を付けて前足をケースの縁へ。**音はオシレータ直打ちを全廃し、効果音10種＋BGMループ＋オン/オフボタンに**（**ファイルは同梱せず WebAudio で合成**。差し替えたいときは `www/assets/audio/` に置いて `USE_FILES=true` にするだけ）。Console エラー0件・3画面を目視。**2026-08-28: Web版(gh-pages)へ反映済み**（Actions の `DEPLOY_FOLDERS` に `nyanko-ice:www` を追加＝以後は push で自動。公開版を実際に開いて確認: フォント2本とも200・Consoleの404は favicon のみ・新しい店内の絵）。**★残り: ①音をオーナーが一度聴く（こちらでは聴けない） ②提出するなら先に `www/support.html` と `www/privacy.html` を作る（Apple必須。このアプリは両方とも未登録＝API で確認）** | 2026-08-28 |
