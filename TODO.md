...
| jyuusetsu-research | サブ | **AI重説調査〜Excel自動入力**（開発中・launchd未割当）。**2026-08-20: 用途地域が常に空だったのを修正**（`XKT001`→**`XKT002`**＋プロパティ名・単位・最寄り代用の100m上限）。ジオコーディングを Google 併用に（`location_type` が ROOFTOP のときだけ採用。加東市で Google が892m外した実測あり）。ストリートビューの節を追加。**e-Statの人口・世帯数も実装済み**（統計表0000020101／A1101・A7101／市区町村コードは国土地理院）。**次は ①Webキーのリファラ制限で社内画面が403 → Console で「Maps Embed だけに制限したキー」を作り `.env.google-maps` に `GOOGLE_MAPS_EMBED_KEY=` を追記（人の作業。コードは対応済み）** | 2026-08-20 **★2026-08-21: 「基本4種（賃貸重説/売買重説/賃貸契約/売買契約）をできるだけ自動作成」へ方針拡大＋3アプリ統合を決裁（着手前）。****実測: 調査で25項目取っているのに書式へ流し込んでいるのは8項目だけ**（賃貸重説にいたっては3項目）。法令制限・災害・周辺環境・価格指標の12項目は取得済みなのに未使用＝ここを埋めるのが最短。**人にお願い: 賃貸契約書と売買重説（土地戸建）の白紙Excel書式が手元に無い**（templates/*.xlsx はgitignore）。 このアプリを母体に`legal-crosscheck`（4点クロスチェック）を④タブとして吸収し、`tokuyaku-generator`の特約機能を③タブとして載せる（**8513は恒久的に残す**＝置き換えではない。170項目カタログはコピーせず共有）。取引種別で③④を出し分け。port は 8536 を予約。**統合で消えるのは legal-crosscheck の1本だけ（53本→52本）**。様式つきの計画は `jyuusetsu-research/TODO.md` の先頭 | 2026-08-21 |
| legal-crosscheck | サブ | **売買書類クロスチェック**（開発中・6/29から停止中）。**2026-08-21: `jyuusetsu-research` の④タブとして吸収する方針で決裁**（＝このアプリは統合後に消える）。吸収時に `_MOCK_MASTERS` を廃止する（キー未設定だと架空の行政データが"正解"として赤字判定の基準になるため）。活かすのは `law_validator.py` の3レイヤー検閲 | 2026-08-21 |
