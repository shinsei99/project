...
| jyuusetsu-research | サブ | **AI重説アシスタント**（開発中・8536予約）。**2026-08-21（夜）: 4書式の検証でバグ2件を修正**（賃貸重説で所在地がハザード欄に入っていた／1セルに2項目の二重割り当て）。**住所の扱いを作り直し**（`所在地`＝住居表示 と `登記所在`＝謄本 を分離。入力は マイソク→謄本→住所 の順で、分かる部分は自動表示。日本郵便で**町名まで**検証）。**マイソク解析を新設**（解析のやり方だけ移植。8505 には触れていない）。**マイソクは鵜呑みにせず謄本・公的データと照合**（🟡 は書類に載せない警告）。**謄本は確定として扱い発行日だけ見る**（30日以内🟢／90日超🟡取り直し。共有 `registry_parser.py` に `証明書発行日` を追加）。**書式の選び方を作り直し**（小分類・単一選択・表示名の短縮・基本セット先頭＋追加は畳む・3種別×売主の立場。**セットが宅建業者売主版を選ぶ不具合を修正**）。**次: ①4書式の印刷イメージを目視 ②災害3項目は `hazard_service` が空を返すスタブで未実装 ③8517 で共有パーサの項目追加を一度確認** | 2026-08-21 |
