...
| keyline | **KeyLine（NFC鍵・備品貸出管理）新規・2026-08-17。** 実装は一通り完成（テスト96件成功・ブラウザ目視済み）。社内LAN限定・本体はメインPC 8534・鍵ボックス横の共用iPhoneがSafariで開くだけ（アプリ導入なし）。**次はNFCタグ到着後の実機検証1点**＝平文httpのLAN URLをiOSのバックグラウンドタグ読み取りが開くか。ここが黒なら方式変更（代替は調査済み） | 2026-08-17 |
