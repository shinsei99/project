...
### ⚠️ PSA 公開API — **存在する。ただし無料枠は1日100回で、いま枯渇している**

Excelに「無料枠あり（**要確認**）」と書いてあったので、実際に叩いて確かめた。

- **エンドポイントは確定**: `GET https://api.psacard.com/publicapi/cert/GetByCertNumber/{cert}`
  （認証は `Authorization: bearer <access token>`。トークンは psacard.com にログインして発行）
- **認証なしで叩くと HTTP 429**。本文が枠をそのまま教えてくれた:

  ```
  "API calls quota exceeded! maximum admitted 100 per Day.
   Please contact collectors-apis@collectors.com"
  retry-after: 79444   ← 約22時間
  ```

  つまり**認証なしの共用枠は1日100回**で、**すでに他の利用者が使い切っている**。
- **登録トークン（無料）の枠は未確認**。登録＝人の作業なので、こちらでは確かめられない。
  第三者の記事では「2026年半ばに大幅削減され、実用には有料プランが要る」とされているが、
  **PSA公式のドキュメントにレート制限の記載は無い**（＝裏が取れていない）。
- **PSAは cert照会しか提供していない。** 相場・population・グレード予測のAPIは無い。

**枚数から見た現実性**（`psa-collection/collection.csv` は **871枚**）:

| 前提 | 一度きりの取り込みにかかる日数 |
|---|---|
| 1日100回が使えるなら | **約9日**（毎日回せば終わる。以後は新規ぶんだけ） |
| 1日1回しか通らないなら | 実質不可能 |

→ **「毎日100回」が登録トークンでも通るなら、無料のまま実用になる。**
やることは「PSAにログインしてトークンを発行し、1日100件ずつ流す」だけ。
**先にトークンを取って、1日の実測値を確かめるのが正解**（それまで有料の判断はしない）。
