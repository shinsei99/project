...
全100面のポリオミノパズル。**ゲーム本体は `www/index.html` の1枚**
（直下の `index.html` は同じ内容の控え。**切り替え帯の1行だけ違う**。直すのは `www/` のほう）。

**★このアプリは6本のゲームの入れ物でもある**（下の帯 `www/games/_switch.js` で行き来する）。
6本で揃える決まりは **[`NEON_STYLE.md`](NEON_STYLE.md)** にある。色・書体・タイトル・音ボタン・
音の調・画面の向きを触るときは、必ずそちらを先に読むこと。
...
### 名前（2026-08-29 に全部「ネオン◯◯」へ統一）

| 変更前 | いまの名前 | 遊び |
|---|---|---|
| ネオンブロックス | **ネオンブロックス**（本体） | 形をはめる |
| カラー・グラビティ | **ネオングラビティ** | 撃ち出して色を混ぜる |
| にゃんこのアイス屋さん | **ネオンアイス** | 積んで揃える |
| にゃんこ大脱出 | **ネオンエスケープ** | 逃げる |
| サイボーグ防衛軍 | **ネオンサイボーグ** | 迎え撃つ |
| ひよこ防衛軍 | **ネオンタワー** | 置いて守る |

**「ネオン＋遊びの種類」で揃えた**ので、6本並んだときに何のゲームか名前だけで分かる。
キャラ（ネコ・ひよこ・サイボーグ）は絵としてそのまま残っている。

**直した場所**: 各ゲームの `<title>` とタイトル画面の表記／切り替え帯（`_switch.js`）／
`CLAUDE.md` の一覧。**フォルダ名（`nyanko-ice` など）は変えていない**
（gitの履歴と `deploy.yml` の対応が切れるため。中身の名前だけ変えている）。
