...
**★他社の掲載について（2026-09-03 オーナー判断で方針変更）**

当初は「**他社スペースの収集はしない**」としていたが、
「他ではどんなワードで検索され、予約に繋がっているのかを比較しないと分からない」という
指摘を受け、**低頻度・手作業の範囲で調べてよい**ことになった。守る線は次の3つ。

- **1回きりの調査として行い、コードにしない。** `spacemarket-monitor` に他社を巡回する
  スクリプトは置かない（実際、2026-09-03 の調査は検索結果を**2回**取得しただけ）
- **定期実行にしない。** launchd にも入れない
- **集めるのは掲載の見せ方（タイトル・価格・定員）まで。** 他社の予約実績や個人情報は扱わない

調査結果は `reports/keywords-2026-09-03.md`。
