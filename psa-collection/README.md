...
- オプション: **ポケモンカード図鑑**（`~/pokecard-dex`）が入っていれば、この中から開ける（下記）
...
日本語版のポケモンカード31,520枚を引ける図鑑（別アプリ `~/pokecard-dex`）を、
サイドバー「表示対象」の **📖 ポケモンカード図鑑** から開ける。

- **図鑑の実体は移していない。** 単独起動（`pokecard-dex/run.sh` → 127.0.0.1:8531）も
  これまでどおり動く。**画面のコードは図鑑側の `app.py` 1本を共有**している
  （こちらに写すと二重メンテになるため）。読み込みは `load_dex()`
- 図鑑が入っていないPCでは**選択肢自体を出さない**。このアプリは図鑑なしでも成立する
- 場所を変えているPCでは環境変数 `POKECARD_DEX` で指定できる
- **保有カードと図鑑カードの自動突き合わせはしない**（2026-08-19 判断）。
  PSA側にはポケモン以外のカード（ONE PIECE等）もあり、CSVは英語表記（YAMATO）・
...
- 保存は `data/albums.json`。図鑑のカードは **`dex:<図鑑キー>`** の形（例 `dex:M6/M6_087`）。
  PSAの証明書番号は数字だけなので接頭辞で見分けられ、**既存のアルバムはそのまま読める**
