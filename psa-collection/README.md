...
- 形式: Streamlit
- port: **8527**
- カテゴリ: ツール（社内LAN共有なし・**launchd 常時起動**＝`com.shinsei.psa-collection`）
- オプション: **ポケモンカード図鑑**（`~/pokecard-dex`）と **ワンピースカード図鑑**
  （`~/onepiece-dex`）が入っていれば、この中から開ける（下記）
...
- 「さらに絞り込む」の中に セット（件数付き）・年・Vault状況・出品状況・PSA推定額のレンジ・
  「保管場所が未記入のものだけ」・カード画像の有無
  - **グレードでの絞り込みは無い**（2026-08-19に外した。使わないため）。
    グレードはカードの表示・並べ替え「グレードが高い順」・集計タブには残っている
  - 常時見えているのは「表示対象・オーダー（提出）・キーワード検索」だけ

## 📖 カード図鑑（オプション・2本）

別アプリの図鑑を、サイドバー「表示対象」から開ける。**どちらもオプション**で、
入っているPCでだけ選択肢に出る（1本も無くてもこのアプリは成立する）。

| 表示 | 実体 | 単独起動 | アルバムでの目印 | 場所を変えたPCでの指定 |
|---|---|---|---|---|
| 📖 ポケモンカード図鑑（31,520枚） | `~/pokecard-dex` | 127.0.0.1:8531 | `dex:` | `POKECARD_DEX` |
| 🏴‍☠️ ワンピースカード図鑑（4,962枚） | `~/onepiece-dex` | 127.0.0.1:8537 | `opdex:` | `ONEPIECE_DEX` |

- 図鑑は `app.py` 冒頭の **`DEXES`（図鑑レジストリ）** で1件ずつ持つ。増やすときは
  ここに1行足す（フォルダ・接頭辞・モジュール名・表示名）。**1本決め打ちはしていない**
- **図鑑の実体は移していない。** 単独起動もこれまでどおり動く。
  **画面のコードは図鑑側の `app.py` 1本を共有**している（こちらに写すと二重メンテになる）。
  読み込みは `load_dexes()`。図鑑はどれもファイル名が `app.py` なので、
  **`sys.modules` には図鑑ごとに別名で登録**する（`pokecard_dex_app` / `onepiece_dex_app`）
- **図鑑側の session_state キーは図鑑ごとに名前空間を分ける**（ワンピは `op_`）。
  2本が同じセッションに載るので、素の `tab` `detail` を使うと画面を行き来したときに
  選択が混ざる（タブ構成も3つと4つで違い、選択肢に無い値が残って例外になる）
- **保有カードと図鑑カードの自動突き合わせはしない**（2026-08-19 判断）。
  PSA側は1枚のCSVにポケモンもワンピも混ざっており、CSVは英語表記（YAMATO）・
  図鑑は日本語（ヤマト）で名前が繋がらない

### 欲しいカード（アルバム連携）

図鑑の各カードの「⭐ 欲しい」で、サイドバーで選んだアルバムに入る。

- 保存は `data/albums.json`。図鑑のカードは **`<接頭辞><図鑑キー>`** の形
  （例 `dex:M6/M6_087` ／ `opdex:OP17-001`）。PSAの証明書番号は数字だけなので接頭辞で
  見分けられ、**既存のアルバムはそのまま読める**（ポケカの `dex:` は今後も変えない）
- 接頭辞での判定（`is_dex()`）は**図鑑がそのPCに入っているかとは無関係に**行う。
  入っていないPCで cert 番号として扱われ、保有カードの枠に紛れ込むのを防ぐため
- バインダーでは **PSAの保有カードと欲しいカードを混ぜず、上下2枠に分けて**並べる
  （🟩HOME ／ 🟦VAULT ／ 🟠WANT）。並べ替えは**枠の中だけ**で、枠をまたぐ移動はしない
- 図鑑の画像は図鑑フォルダから借りる。**DBの画像パスは相対**（`data/myca_large/…`）なので、
  図鑑側の `_p()` で図鑑フォルダ基準に直してから読む（cwd が違うと1枚も出ない）
...
> 元の見出し: 「PSAカード管理（psa-collection）補足 ※ツール・port 8527」
...
- **カード画像**（🖼ギャラリータブ）: **918枚取得済み＝CSVの全件**（保有357＋売却済561・464MB。2026-09-02時点）。`data/images/<cert>.jpg`に永久キャッシュ、一覧用サムネは`data/thumbs/`に初回自動生成。
...
- **★`osascript -e` の中のJSに `"\n"` と書いてはいけない（2026-09-02・再調査不要）**: AppleScript は `\n` を**本物の改行に展開してから** JS に渡すので、`join("\n")` は「文字列リテラルの途中で改行」＝ JS の SyntaxError になる。`do JavaScript` はこれを**空文字・rc=0・stderr も空**で返すため、`fetch_new_images.sh` が**無言で「収集 0 件」**になり、アプリは「画像0枚を取得しました」と**成功表示**していた。区切りは必ず `String.fromCharCode(10)` で作る。あわせて 0件なら非ゼロ終了するようにし、アプリ側も取得後に不足枚数を数え直して**減っていなければ警告**を出すようにした。
- **★収集対象は保有中だけでなく売却済も要る（2026-09-02）**: `harvest_collectors.js` の `STATUS` が `["ACTIVE"]` 固定だった一方、アプリの「画像が無いカード」判定は**保有・売却を問わない**ため、売却済の新規カードは永久に埋まらなかった。既定を `["ACTIVE","SOLD"]` に変更（絞りたいときは流し込む前に `window.__hStatus` を入れる）。全件ページングになるぶん時間がかかるので、`fetch_new_images.sh` の待ちは40秒→150秒。**途中結果は使わず、時間内に終わらなければ失敗にする**。
- **★PSAのCSVは `Item Status` の書き方が変わる（2026-09-02・再調査不要）**: 2026-09-01 の書き出しから **売却済が `Sold` → `Previously Owned`** になった。`app.py` が `== "Sold"` の決め打ちだったため、**売却済ビューが 561枚 → 0枚**になっていた（画像が出ない原因のもう半分）。判定は `ACTIVE_STATUSES` / `SOLD_STATUSES` と `is_active()` / `is_sold()` に集約したので、**また書き方が変わったらこの2つのタプルに足すだけ**でよい。
