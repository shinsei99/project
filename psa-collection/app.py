...
if not CSV_PATH.exists():
    ...
    st.info("サイドバーの「データ更新」からPSAエクスポートCSVをアップロードしてください。")
    st.stop()
...
status = st.sidebar.radio(
    "表示対象", ["保有中", "売却済", "すべて"], horizontal=True,
)
if status == "保有中":
    view = df[df["Item Status"] == "Active"].copy()
elif status == "売却済":
...
if is_sold_view:
    c2.metric("売却額 合計", yen(view["Sold Price"].sum()))
    c3.metric("手取り 合計", yen(view["Sold Proceeds"].sum()))
    fees = view["Sold Fees"].sum()
    rate = fees / view["Sold Price"].sum() * 100 if view["Sold Price"].sum() else 0
    c4.metric("手数料 合計", yen(fees), f"売却額の{rate:.1f}%", delta_color="off")
else:
        ...
        for start in range(0, len(page_df), per_row):
            ...
            for col, (_, card) in zip(st.columns(per_row), chunk.iterrows()):
                ...
                with col:
                    ...
                    price = card["Sold Price"] if is_sold_view else card["PSA Estimate"]
                    st.markdown(
                        f"**PSA {card['Grade']}**　{yen(price)}  \n"
                        f"{card['Subject'] or ''}  \n"
                        f"<span style='font-size:11px;color:#888'>"
                        f"{card['Year'] or ''} {card['Set'] or ''}<br>"
                        f"{('📦 ' + card['保管場所']) if card['保管場所'].strip() else ''}"
                        f"</span>  \n"
...
with tab_list:
    if view.empty:
        ...
        cols += (
            ["Sold Price", "Sold Proceeds", "Sold Date", "Sold On"]
            if is_sold_view
