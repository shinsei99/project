...
1. `ai-tools-base/CLAUDE.md` … **この1本の運用ルール**（1本増やしたらZenn・noteも同時に出す）
2. `ai-tools-base/SESSION_LOG.md` … 2026-08-16 の3節が今回ぶん（上が新しい）
3. `ai-tools-base/drafts/README.md` … 3媒体の書き分けと、出さないと決めたもの
4. `ai-tools-base/drafts/PUBLISH.md` … **公開待ちの順番表**（Zennは1日2本まで）
...
- Zenn公開用のディレクトリ … **リポジトリ直下の `articles/`**（`ai-tools-base/` の中ではない）
...
| `.next/` `out/` | ビルドで再生成 |
| **`.vercel/`** | **`npx vercel link` でプロジェクト `ai-tools-base`（team: brain-dump）に紐づける** |
