...
| registry-annex-building（区分所有の謄本＝本体と車庫） | `zenn/registry-pdf-merge-overwrite.md` ✅公開 | `note/touhon-ga-nimai.md` ✅公開 | `/works/registry-annex-building` ✅公開 |
| mobile-photo-upload（スマホ送信が3枚目で失敗＝4.5MB上限） | `zenn/ios-safari-vercel-upload-413.md` ✅公開 | `note/sanmai-me-de-tomaru.md` ✅公開 | `/works/mobile-photo-upload` ✅公開 |
...
- **theta-viewer は保留（2026-08-16 → 2026-08-18 に状況を更新）。**
  **メインPCでは完成している**（本人確認）。サブPCに来ていないのは**ドキュメントだけ**で、
  `README.md` は Vite の雛形のまま・`SESSION_LOG.md` と `TODO.md` は存在しない。
  **`.gitignore` の `!theta-viewer/**` で許可されているため git で渡せる**（実測確認済み）。
  → **メインPCがドキュメントを書いて push したら、それを読んで制作記録を書く。**
  憶測で埋めない。依頼は直下の `TODO.md` に記載済み
