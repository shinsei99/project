...
# ★静止期間（2026-08-30）。drafts/zenn_quiet_until.txt に書いた日時までは**何もしない**。
#   なぜ要るか: 上限に当たっている間、毎晩の投稿・空コミットの促しが**それ自体で
#   「直近24時間の投稿数」を食い直している疑いがある**（8/26から4日間、20回以上デプロイして
#   1本も通らなかった）。抜けるには「一度も試みない時間」を作るしかない。
#   ★launchd を外す形にはしない。外すと戻し忘れて永久に止まる。
#     日時を過ぎれば**勝手に復帰する**この形にしてある。前倒しで再開したいならファイルを消す。
QUIET="drafts/zenn_quiet_until.txt"
if [ -f "$QUIET" ]; then
  UNTIL="$(grep -v '^#' "$QUIET" | head -1 | tr -d ' \t')"
  NOW="$(date '+%Y-%m-%dT%H:%M')"
  if [ -n "$UNTIL" ] && [ "$NOW" \< "$UNTIL" ]; then   # ISO表記なので文字列比較で正しく並ぶ
    echo "$(date '+%F %T') 静止期間中（$UNTIL まで）。今夜は投稿も再デプロイの促しもしない。"
    echo "  理由: $(grep '^#' "$QUIET" | head -3 | tr '\n' ' ')"
    exit 0
  fi
  # ${} 必須: 直後が全角括弧だと bash が変数名に巻き込む（run_ocr_nightly.sh と同じ罠）
  echo "$(date '+%F %T') 静止期間（${UNTIL}）を過ぎたので再開する。"
fi
