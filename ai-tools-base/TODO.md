## 🛑 Zenn が5日以上出せない — ★原因が特定できた（2026-08-31 夜）

**Zennのデプロイ画面が「投稿数の上限に達したためデプロイされませんでした」と明言。**
そのうえで公式FAQ（https://zenn.dev/faq/rate-limit）にこう書いてある:

> 記事は**直近24時間以内の投稿数（投稿予約中を含む）**に基づいて判定されます。

**★ここが効いている。** 2026-08-26 08:54 のコミット `6076e55c` で、
**22本に `published_at` を振って 2026-08-27〜09-17 の毎日22:30に公開予約**した。
Zennがこれを受け取っていれば、**9/17まで毎日「24時間以内の予約」が存在し続ける**ので、
待っても上限が空かない。「24時間空ければ解ける」が効かなかったのはこれで説明がつく。

手元の `published_at` は既に外してあるが、**Zenn側に残った予約は消えない**
（弾かれたデプロイでは更新されないため）。実際 `deploy-not-reflected` のURLは
**403（存在するが非公開）**を返す＝Zenn側に記事の実体がある。

### ★人にしかできない対処（ログインが要る）

1. https://zenn.dev/dashboard で**記事の管理**を開く
2. **公開予約（投稿予約中）になっている記事**を探す（8/26に作られた22本が候補）
3. **予約を取り消す**（下書きに戻す）か**削除する**
4. 翌日、`curl -s "https://zenn.dev/api/articles?username=shinsei99&count=100" | grep -c slug`
   が 12 になっていれば解けた

**これで解けなければ**、公式が案内している唯一の窓口へ申請する:
> 他ブログサービスなどからZennにコンテンツを移行する場合、一時的に上限を緩和できます。
> **お問い合わせフォームより申請してください。**
文案は `drafts/zenn-inquiry.md`（そのまま貼れる）。

### ついでに直した

- デプロイのたびに出ていた「booksディレクトリが見つかりませんでした」を止めた
  （空の `books/.keep` を置いた）。**本当の失敗を見落とさないため**
